...
def send_welcome_email(sender, user, **kwargs):
    ...
    send_mail(subject, content, 'nduduzompumelelokhanyile@gmail.com', [user.email])
...
def customer_signup_view(request):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and customerForm.is_valid():
            ...
            # Save the user first to get the user's email address
            ...
            send_welcome_email(sender=None, user=user, **{})
...
#---------------------------------------------------------------------------------
#------------------------ END OF PUBLIC CUSTOMER RELATED VIEWS -------------------
#---------------------------------------------------------------------------------
...
def process_payment(request):
    ...
    if 'product_ids' in request.COOKIES:
        ...
        if product_ids != "":
            ...
            for p in products:
                total=total+p.price
    return render(request,'ecom/payment.html',{'products':products,'total':total,'product_count_in_cart':product_count_in_cart})
...
def payment_success_view(request):
    ...
    # Retrieve email, mobile, and address from cookies
    email = request.COOKIES.get('email')
    mobile = request.COOKIES.get('mobile')
    address = request.COOKIES.get('address')

    # Place orders and send confirmation email
...
def generate_invoice_pdf(order):
    # Step 1: Render the Invoice Template
    template = get_template('ecom/invoice_template.html')
...
def invoice_pdf_view(request, order_id):
    ...
    pdf_path = order.invoice_file.path  # Path to the PDF file
...
def send_order_confirmation_email(order):
    ...
    # Step 4: Store the PDF (Example: Save it in the media directory)
    ...
    invoice_url = reverse('invoice_pdf', kwargs={'order_id': order.id})

    subject = 'Order Confirmation: #{}'.format(order.id)  # Use the order ID in the subject
